...
#     quit()
#Webhook Discord Bot
hook = Webhook("https://discordapp.com/api/webhooks/783876113574527017/oyAnGNPn0hQGu877yZCAXiF58KIIsokw5SkZ-eX0V17v_edHYjHbPGordF4IHz4mrNX_")
...
def weekday_candlestick(stock, ohlc_data, closep, openp, mom, direction, Av1, Av2, date, SP, df, fmt='%b %d', freq=50, **kwargs):
    ...
    Label1 = '5 SMA'
    Label2 = '13 SMA'
    ax.plot(ndays[4:], Av1, '#FFFFFF',label=Label1, linewidth=0.8, alpha=0.8)
    ax.plot(ndays[12:], Av2, '#3798FF',label=Label2, linewidth=0.8, alpha=0.8)
    ...
    ax2.plot(ndays, mom, '#FF3431', label='Momemtum', linewidth=1)
    ...
    ax2.set_xlim(12, ndays.max()+1)
    ax2.tick_params(axis='x', colors='w')
    ax2.tick_params(axis='y', colors='w')
    plt.ylabel('Momemtum', color='w')
    ...
    hook.send('Momemtum Swing Alert: ' + stock + '  |  Frequency: 1 Hour' + '\n' + 'Direction: ' + direction, file=discord_pic)
    plt.close(fig)
...
def graphData(stock, MA1, MA2):
    try:
        ...
        if (mom.iat[-3] < 0 and mom.iat[-2] < 0) and mom.iat[-1] > 0:
            ...
            weekday_candlestick(stock, newAr, closep, openp, mom, direction, Av1, Av2, date, SP, df, fmt='%b %d', freq=3, width=0.5, colorup='green', colordown='red', alpha=1.0)
