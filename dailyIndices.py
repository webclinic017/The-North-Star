...
#Webhook Discord Bot
hook = Webhook("https://discordapp.com/api/webhooks/778077407138938881/yvMzM29T3_iJBGpPjNyJKGmmIjCToAIzG3KOelmWKLlNWOesq598IE25Os6uHVOBEWi3")
...
def weekday_candlestick(stock, sector, ohlc_data, closep, openp, volume, Av1, Av2, date, SP, df, fmt='%b %d', freq=50, **kwargs):
    ...
    hook.send("RSI ALERT: " + str(sector) + "  Frequency: Daily", file=discord_pic)
    plt.close(fig)
...
def graphData(stock,sector, MA1, MA2):
    try:
        df = pdr.get_data_yahoo(stock, period = "6mo", interval = "1d", retry=20, status_forcelist=[404, 429, 500, 502, 503, 504], prepost = True)
        #print(df)
        #df.reindex(df.index)
