...
def update_graph_scatter(n): 
    ...
    BuySide = go.Candlestick(
        x=list(xcol),
        open=list(openn),
        high=list(closeArray),
        low=list(closeArray),
        close=list(closeArray),
        increasing={'line': {'color': '#00CC94'}},
        decreasing={'line': {'color': '#F50030'}},
        name='candlestick')
    # data = plotly.graph_objs.Scatter( 
    ...
    figure = dict({'data': [BuySide], 
             'layout' : go.Layout(xaxis=dict(range=[min(xcol),max(xcol)]),yaxis = dict(range = [min(ycol),max(ycol)])),'xaxis': {'rangeslider': {'visible': False}})
#go.Layout(xaxis=dict(range=[min(xcol),max(xcol)]),yaxis = dict(range = [min(ycol),max(ycol)]))} )
    #fig = go.Figure(data=[go.Scatter(x=list(xcol), y=list(ycol))], layout=[go.Layout(xaxis=dict(range=[min(xcol),max(xcol)]),yaxis = dict(range = [min(ycol),max(ycol)]))])

    #layout = go.Layout(xaxis=dict(range=[0,20]),yaxis=dict(range=[min(ycol),max(ycol)]))
    # layout = {'layout' : go.Layout(xaxis=dict(range=[min(xcol),max(xcol)]),yaxis = dict(range = [min(ycol),max(ycol)]),)} 
    # app.layout = layout
