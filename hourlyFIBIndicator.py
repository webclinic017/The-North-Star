...
def weekday_candlestick(stock, ohlc_data, closep, openp, fib,  Av1, Av2, date, SP, df, fmt='%b %d', freq=50, **kwargs):
...
def graphData(stock, MA1, MA2):
    try:
        ...
        stock = "{}".format(stock)
        df = pdr.get_data_yahoo(stock, period = "5d", interval = "1h", retry=20, status_forcelist=[404, 429, 500, 502, 503, 504], prepost = True)
        ...
        weekday_candlestick(stock, newAr, closep, openp, fib, Av1, Av2, date, SP, df, fmt='%b %d', freq=3, width=0.5, colorup='green', colordown='red', alpha=1.0)
