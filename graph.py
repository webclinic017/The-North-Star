...
def graphDailyMF(ticker):
    ticker = "{}".format(ticker)
    # valid periods: 1d,5d,1mo,3mo,6mo,1y,2y,5y,10y,ytd,max
    data = pdr.get_data_yahoo(ticker, period = "6mo", interval = "1d", retry=20, status_forcelist=[404, 429, 500, 502, 503, 504], prepost = True)
    graphdataMF(ticker, data, 5, 13)

def graphhrMF(ticker):
    ticker = "{}".format(ticker)
    # valid periods: 1d,5d,1mo,3mo,6mo,1y,2y,5y,10y,ytd,max
    data = pdr.get_data_yahoo(ticker, period = "5d", interval = "1h", retry=20, status_forcelist=[404, 429, 500, 502, 503, 504], prepost = True)
    graphdataMF(ticker, data, 5, 13)

def graphDailyFT(ticker):
    ticker = "{}".format(ticker)
    # valid periods: 1d,5d,1mo,3mo,6mo,1y,2y,5y,10y,ytd,max
    data = pdr.get_data_yahoo(ticker, period = "6mo", interval = "1d", retry=20, status_forcelist=[404, 429, 500, 502, 503, 504], prepost = True)
    graphdataFT(ticker, data, 5, 13)

def graphhrFT(ticker):
    ticker = "{}".format(ticker)
    # valid periods: 1d,5d,1mo,3mo,6mo,1y,2y,5y,10y,ytd,max
    data = pdr.get_data_yahoo(ticker, period = "5d", interval = "1h", retry=20, status_forcelist=[404, 429, 500, 502, 503, 504], prepost = True)
    graphdataFT(ticker, data, 5, 13)
...
def weekday_candlestickV(stock, ohlc_data, closep, openp, vortex, Av1, Av2, date, SP, df, fmt='%b %d', freq=50, **kwargs):
    ...
    fig.savefig('hourPics/' + stock + '.png', facecolor=fig.get_facecolor())
    discord_pic = File('hourPics/' + stock + '.png')
    hook.send('Vortex Indicator for Ticker: ' + stock, file=discord_pic)
    plt.close(fig)
 
def weekday_candlestickMF(stock, ohlc_data, closep, openp, moneyFlow,  Av1, Av2, date, SP, df, fmt='%b %d', freq=50, **kwargs):
    """ Wrapper function for matplotlib.finance.candlestick_ohlc
        that artificially spaces data to avoid gaps from weekends """
    # Convert data to numpy array
    ohlc_data_arr = np.array(ohlc_data)
    ...
    ax2 = plt.subplot2grid(
        (6, 4), (4, 0), sharex=ax, rowspan=2, colspan=4, facecolor='#07000d')
    ax2.plot(ndays[14:], moneyFlow[14:], '#FFFF', linewidth=1.5)
  
    ax2.axhline(y=85, linewidth=1, color='r')
    ax2.axhline(y=15, linewidth=1, color='g')
    ...
    fig.savefig('hourPics/' + stock + '.png', facecolor=fig.get_facecolor())
    discord_pic = File('hourPics/' + stock + '.png')
    hook.send("Money Flow Indicator for Ticker: " + stock, file=discord_pic)
    plt.close(fig)
    
def weekday_candlestickFT(stock, ohlc_data, closep, openp, fish, Av1, Av2, date, SP, df, fmt='%b %d', freq=50, **kwargs):
    """ Wrapper function for matplotlib.finance.candlestick_ohlc
        that artificially spaces data to avoid gaps from weekends """
    # Convert data to numpy array
    ohlc_data_arr = np.array(ohlc_data)
    ...
    fig.savefig('hourPics/' + stock + '.png', facecolor=fig.get_facecolor())
    discord_pic = File('hourPics/' + stock + '.png')
    hook.send("Fisher Transform Indicator: " + stock, file=discord_pic)
    plt.close(fig)
...
def graphdataMF(stock, data, MA1, MA2):
    df = data
    df = df.reset_index()
    df.index = pd.to_datetime(df.index)
    df.index.name = 'Date'
    ...
    weekday_candlestickMF(stock, newAr, closep, openp, moneyFlow, Av1, Av2, date, SP, df, fmt='%b %d', freq=3, width=0.5, colorup='green', colordown='red', alpha=1.0)

def graphdataFT(stock, data, MA1, MA2):
    df = data
    df = df.reset_index()
    df.index = pd.to_datetime(df.index)
    df.index.name = 'Date'
    ...
    weekday_candlestickFT(stock, newAr, closep, openp, fish, Av1, Av2, date, SP, df, fmt='%b %d', freq=3, width=0.5, colorup='green', colordown='red', alpha=1.0)
