...
#Webhook Discord Bot
hook = Webhook("https://discordapp.com/api/webhooks/751543331372400786/aU5RzVxF9AiMxkMgJyLAAhqOr6FPucdI4mHrcEqfxmtHlWafNCKb9AkqN_G2ZbMvcya7")
...
def weekday_candlestick(stock, ohlc_data, closep, openp, moneyFlow,  Av1, Av2, date, SP, df, fmt='%b %d', freq=50, **kwargs):
    ...
    hook.send("Money Flow Indicator ALERT: " + stock + "  Frequency: Daily", file=discord_pic)
    plt.close(fig)
...
def graphData(stock, MA1, MA2):
    try:
        ...
        if moneyFlow.iloc[-1] > 85 or moneyFlow.iloc[-1] < 15:
            ...
            weekday_candlestick(stock, newAr, closep, openp, moneyFlow, Av1, Av2, date, SP, df, fmt='%b %d', freq=3, width=0.5, colorup='green', colordown='red', alpha=1.0)
