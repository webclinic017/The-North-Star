...
#     quit()
#Webhook Discord Bot
hook = Webhook("https://discordapp.com/api/webhooks/786002693545394177/QSiFhjCYyF-thBmZYpP7osA7PwdNR1s-YURRoIfxy_1UPLgTHUsjZHjKmfUxlNjU8Z0H")
...
def weekday_candlestick(stock, ohlc_data, closep, openp, volume, low, high, Av1, Av2, date, SP, df, fmt='%b %d', freq=50, **kwargs):
    ...
    ax.axhline(low,xmin=0,xmax=ndays.max()+1, color='w', linewidth=0.5, label='Previous 2 Day Low')
    ax.axhline(high,xmin=0,xmax=ndays.max()+1, color='w', linewidth=0.5, label='Previous 2 Day High')
    ...
    hook.send('Double Inside Day Alert: ' + '**' + stock + '**' +  '  Frequency: Daily', file=discord_pic)
    plt.close(fig)
...
def graphData(stock, MA1, MA2):
    try:
        ...
        if (highp.iat[-1] < highp.iat[-2] and lowp.iat[-1] > lowp.iat[-2]) and (highp.iat[-2] < highp.iat[-3] and lowp.iat[-2] > lowp.iat[-3]):
